...
def get_bvp_features(data: pd.Series, window_length: int = 60, window_step_size: float = 1.0, data_frequency: int = 64,
                     num_cores: int = 1):
    """
    Computes statistical BVP features.

    Parameters
    ----------
    data : pd.DataFrame
        input BVP time series 
    window_length : int
    ...
    Returns
    -------
    BVP Features: pd.DataFrame
        A DataFrame containing all ststistical features.

    Notes
    -----
    DataFrame contains the following BVP features
    ...
    Examples
    --------
    >>> bvp_features = flirt.bvp.get_bvp_features(bvp, 60)
    """
