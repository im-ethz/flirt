...
pf = flirt.eda.preprocessing.ParticleFilter(num_particles=10, num_smoothers=5)
...
filtered_pf = pf.__process__(eda)
...
ax1.set_title('Butterworth Low-Pass Filtered EDA')
...
plt.legend(bbox_to_anchor=(1.1, 1.1), loc=1, frameon=True, columnspacing=0.1, labelspacing=0.1,
    fancybox=True)

ax2 = plt.subplot(312)  
plt.plot(time, np.array(eda), 'y-', linewidth = 1, alpha = 1, label='Raw EDA')
#plt.plot(time, np.array(phasic_cvx_lr), 'b-', linewidth = 1, alpha = 1, label='Phasic Data cvx')  
plt.plot(time, np.array(filtered_ekf), 'b-', linewidth = 0.8, alpha = 0.8, label='EDA Filtered with EKF')  
...
ax2.set_title('Kalman Filtered EDA')
...
plt.plot(time, np.array(filtered_pf), 'b-', linewidth = 0.8, alpha = 0.8, label='EDA Filtered with PF')  
#plt.plot(time, np.array(tonic_leda_lr), 'r--', linewidth = 0.8, alpha = 0.8, label='Tonic Data ledalab')
ax3.set_title('Particle Filtered EDA')
...
print('done plotting')
plt.tight_layout()
plt.savefig('lpf_ekf_pf.pdf', dpi=300, bbox_inches='tight', transparent=False)
