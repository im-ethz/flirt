...
$\mathbf{x} = [SC_H, k_{diff}, SC_0, SCR, S]^T$, where $SC_H$ is the hydration-dependent contribution to the SC, $SC_0$ is the baseline SC of inert skin, $SCR$ is the SCR to sudomotor activity, $k_{diff}$ is the inverse time constant of the sweat diffusion, and $S$ is the sudomotor nerve activation. $k_{diff}$ and $SC_0$ are assumed constant within an experiment but are expected to vary between experiments.
...
\begin{enumerate}
    \item \textbf{The prediction step}: prior update of the mean and variance of the state's probability distribution, using the SC model
    \item \textbf{The measurement update}: posterior update of the mean and variance of the state's probability distribution, using the SC measurement
\end{enumerate}

The final estimate of the SC signal is retrieved as $SC = SC_H + SC_0 + SCR$.

From an implementation perspective, the mean was initialised as a vector of zeros: $\mathbf{x_0} = [0, 0, 0, 0, 0]^T$, and the variance was initialised as:
\begin{equation*}
...
\end{bmatrix} 
\end{equation*}
The parameters that control the amount of noise filtering and artifact removal are: the sudomotor nerve activation (S) response amplitude (\textit{alpha}), the minimum and maximum rates of change for sudomotor response (\textit{d-min} and \textit{d-max}), the minimum and maximum absolute difference between two consecutive measurements (\textit{min-diff} and \textit{max-diff}), and the minimum and maximum Savitsky-Golay differential (\textit{min-Savi-Go} and \textit{max-Savi-Go}). The default values for these main  parameters are chosen to achieve the best results outside the clinic. Nevertheless, they can be changed to fit the user's needs, according to the following considerations:  
\begin{itemize}
    \item Decreasing \textit{alpha} tends to filter out more of the noise and reduce the phasic amplitude response.
    \item Increasing the gap between \textit{min-diff} and \textit{max-diff}, and between \textit{min-Savi-Go} and \textit{max-Savi-Go}, reduces the amount of artifact suppression, as the Kalman gain is non-zero for measurements within these ranges. A non-zero Kalman gain puts more emphasis on the potentially erroneous measurement when performing the signal estimate. 
    \item Increasing (decreasing) \textit{d-max} (\textit{d-min}) reduces the amount of artifact and noise suppression, as higher (lower) rates of change in sudomotor response are generally attributed to artifacts or noise. 
\end{itemize}
