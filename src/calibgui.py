...
from src.calib import calib_1cam, calib_2cam, calib_ncam
from src.utils import (
    get_calibrated_points,
    get_normalized_points_dict,
    get_precal_coords,
    get_timestamp,
    saved_to_info,
)

img_filetypes = (('PNG files', '*.png'), ('JPG files', '*.jpg'), ('JPEG files', '*.jpeg'), ('All files', '*.*'))

npy_filetypes = (('NPY Files', '*.npy'), ('All files', '*.*'))
...
class CalibGUI(tk.Frame):
    def __init__(self, master=None, args={}):
        ...
        master.geometry(f'{main_window_width}x{main_window_height}')
        ...
        self.button_images = ttk.Button(master, text='Images', command=self.select_img_files).grid(
            column=0, row=curr_row, sticky=tk.W + tk.E, padx=10, pady=10
        )

        # Button Select minimap
        self.button_minimap = ttk.Button(master, text='Minimap', command=self.select_minimap).grid(
            column=1, row=curr_row, sticky=tk.W + tk.E, padx=10, pady=10
        )
        curr_row += 1

        # Button load points
        self.button_images = ttk.Button(master, text='Points', command=self.load_point_data).grid(
            column=0, row=curr_row, sticky=tk.W + tk.E, padx=10, pady=10
        )

        # Button load parameters
        self.button_images = ttk.Button(master, text='Parameters', command=self.load_parameters).grid(
            column=1, row=curr_row, sticky=tk.W + tk.E, padx=10, pady=10
        )
        curr_row += 1

        # Window 1 Text box
        tk.Label(master, text='Image 1').grid(column=0, row=curr_row, sticky=tk.W + tk.E, columnspan=2, padx=10, pady=0)
        curr_row += 1

        # File List boxes
        self.file_list_img1 = tk.Listbox(master, height=5, width=20, selectmode='extended')
        self.file_list_img1.grid(column=0, row=curr_row, sticky=tk.W + tk.E, columnspan=2, padx=10, pady=10)
        curr_row += 1

        # Window 2 Text box
        tk.Label(master, text='Image 2').grid(column=0, row=curr_row, sticky=tk.W + tk.E, columnspan=2, padx=10, pady=0)
        curr_row += 1

        self.file_list_img2 = tk.Listbox(master, height=5, width=20, selectmode='extended')
        self.file_list_img2.grid(column=0, row=curr_row, sticky=tk.W + tk.E, columnspan=2, padx=10, pady=10)
        curr_row += 1

        # Point list text box
        tk.Label(master, text='Points').grid(column=0, row=curr_row, sticky=tk.W + tk.E, columnspan=2, padx=10, pady=0)
        curr_row += 1
        ...
        self.point_list = tk.Listbox(
            master, height=5, width=20, selectforeground='red', selectmode='extended', activestyle='none'
        )
        self.point_list.grid(column=0, row=curr_row, sticky=tk.W + tk.E, columnspan=2, padx=10, pady=10)
        curr_row += 1

        # Image & minimap scaling option menu
        tk.Label(master, text='Scale Image').grid(column=0, row=curr_row, sticky=tk.W + tk.E, padx=10, pady=0)
        tk.Label(master, text='Scale Minimap').grid(column=1, row=curr_row, sticky=tk.W + tk.E, padx=10, pady=0)
        curr_row += 1
        ...
        self.img_scaling_option_menu = tk.OptionMenu(master, self.img_scale, *img_scale_opt)
        self.img_scaling_option_menu.grid(column=0, row=curr_row, sticky=tk.W + tk.E, padx=10, pady=10)
        ...
        self.minimap_scaling_option_menu = tk.OptionMenu(master, self.minimap_scale, *minimap_scale_opt)
        self.minimap_scaling_option_menu.grid(column=1, row=curr_row, sticky=tk.W + tk.E, padx=10, pady=10)
        curr_row += 1

        # Calibrate Buttons
        self.button_calibrate_2cams = ttk.Button(master, text='Calibrate 2 Cameras', command=self.calibrate_2cams).grid(
            column=0, row=curr_row, sticky=tk.W + tk.E, columnspan=2, padx=10, pady=10
        )
        curr_row += 1

        # Calibrate Buttons
        self.button_calibrate_ncams = ttk.Button(master, text='Calibrate N Cameras', command=self.calibrate_ncams).grid(
            column=0, row=curr_row, sticky=tk.W + tk.E, columnspan=2, padx=10, pady=10
        )
        curr_row += 1

        # Calibrate Buttons
        self.button_recalibrate = ttk.Button(master, text='Add 1 Camera', command=self.calibrate_1cam).grid(
            column=0, row=curr_row, sticky=tk.W + tk.E, columnspan=2, padx=10, pady=10
        )
        curr_row += 1

        self.button_eval_calibration = ttk.Button(
            master, text='Evaluate Calibration', command=self.display_image_points_to_minimap_points
        ).grid(column=0, row=curr_row, sticky=tk.W + tk.E, columnspan=2, padx=10, pady=10)
        curr_row += 1

        self.button_save = ttk.Button(master, text='Save', command=self.save).grid(
            column=0, row=curr_row, sticky=tk.W + tk.E, padx=10, pady=10
        )
        ...
        self.canvas_img2 = tk.Canvas(window_img2, width=self.canvas_width, height=self.canvas_height)
        ...
        self.canvas_minimap = tk.Canvas(window_img3, width=self.canvas_width, height=self.canvas_height)
    ...
    def select_img_files(
        self,
    ):
        img_filepaths = fd.askopenfilenames(filetypes=img_filetypes, parent=self.master, title='Choose a file')
        ...
        for idx, f_name in enumerate(self.img_files):
            self.file_list_img1.insert(tk.END, f'{idx} : {f_name}\n')
            self.file_list_img2.insert(tk.END, f'{idx} : {f_name}\n')
    ...
    def add_point_img1(self, event):  # on click
        ...
        for img_lbl in self.point_data.keys():
            point_labels += [int(i) for i in self.point_data[img_lbl].keys()]
    ...
    def delete_selected_point_in_image(self, event):
        ...
        if self.point_lbl not in point_labels:
            # Delete from point list
            self.point_list.delete(self.point_idx)
    ...
    def update_point_list(self):
        ...
        point_labels = []
        for img_lbl in self.point_data.keys():
            point_labels += self.point_data[img_lbl].keys()
        point_labels = sorted(set(point_labels))
        ...
        for p_idx, p_lbl in enumerate(point_labels):
            if p_lbl not in point_list_entries:
                self.point_list.insert(p_idx, f'{p_lbl}\n')
    ...
    def redraw_images(self):
        ...
        self.img1 = load_image(file_path_img1, scale=float(self.img_scale.get()))
        self.canvas_img1.create_image(0, 0, anchor=tk.NW, image=self.img1, tags='img1')
        self.canvas_img1.update()

        # Canvas 2
        self.canvas_img1.delete('img2')
        file_path_img2 = self.img_filepaths[self.idx_img2]
        self.img2 = load_image(file_path_img2, scale=float(self.img_scale.get()))
        self.canvas_img2.create_image(0, 0, anchor=tk.NW, image=self.img2, tags='img2')
        self.canvas_img2.update()

    def redraw_minimap(self):
        # Canvas 3
        self.canvas_minimap.delete('minimap')
        self.minimap = load_image(self.minimap_path, scale=float(self.minimap_scale.get()))
        self.canvas_minimap.create_image(0, 0, anchor=tk.NW, image=self.minimap, tags='minimap')
    ...
    def redraw_points(self):
        ...
        for img_lbl in self.point_data.keys():
            pt_labels = [i for i in self.point_data[img_lbl].keys()]
            for p_lbl in pt_labels:
                if img_lbl == self.idx_img1:
                    ...
                    if p_lbl == self.point_lbl:
                        ...
                        color = 'blue'
                    self.canvas_img1.create_oval(x - 2, y - 2, x + 2, y + 2, fill=color, width=0, tags='point')
                ...
                if img_lbl == self.idx_img2:
                    ...
                    if p_lbl == self.point_lbl:
                        ...
                        color = 'blue'
                    self.canvas_img2.create_oval(x - 2, y - 2, x + 2, y + 2, fill=color, width=0, tags='point')
                ...
                if img_lbl == self.idx_minimap:
                    ...
                    if p_lbl == self.point_lbl:
                        ...
                        color = 'blue'
                    self.canvas_minimap.create_oval(x - 2, y - 2, x + 2, y + 2, fill=color, width=0, tags='point')
    ...
    def save(self, *args):
        ...
        # Write setting into file
        setting_file_path = os.path.join(state_folder_path, 'setting.txt')
        # Open file
        with open(setting_file_path, 'w') as f:
            for fname in self.img_files + [self.minimap_filename]:
                f.write(fname + '\n')
    ...
    def load_parameters(
        ...
        parameter_filepaths = fd.askopenfilenames(filetypes=npy_filetypes, parent=self.master, title='Choose file')
        ...
        for param_filepath, param_filename in zip(parameter_filepaths, param_file_names):
            ...
            if param_file_base in file_base:
                ...
                if idx not in self.calibrated:
                    self.calibrated.append(idx)
            else:
                print(f'Ignoring paramter: {param_filename}')
    ...
    def calibrate_2cams(
        ...
        for calib_idx in parameters_dict.keys():
            if calib_idx not in self.calibrated:
    ...
    def calibrate_1cam(
        ...
        calibrated_points = get_precal_coords(
            self.idx_img1, self.calibrated, self.calib_param, normalized_point_info, self.floor_points
        )

        parameters = calib_1cam(
            self.idx_img1, calibrated_points, normalized_map_point_info, self.n_cam, normalized_saved
        )

        if self.idx_img1 not in self.calibrated:
    ...
    def display_image_points_to_minimap_points(self):
        ...
        if self.idx_img1 in self.calib_param.keys():
            ...
            print(f'No Camera calibration available for: {self.idx_img1}')
        ...
        if self.idx_img2 in self.calib_param.keys():
            ...
            print(f'No Camera calibration available for: {self.idx_img2}')
